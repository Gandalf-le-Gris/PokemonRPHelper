...
import { talentArray, TalentType } from '@/types/talents';
...
const talentsWithMods = computed(() => {
  const mods = JSON.parse(JSON.stringify(character.value.talents.filter(talent => talent.mod > 0))) as {talent: TalentType, mod: number}[]
  computeGlobalModifiers(character.value).talents?.map(t => { return {talent:t.name, mod: t.mod}})
    .forEach(t => {
      const i = mods.findIndex(m => m.talent === t.talent)
      if (i != -1) {
        mods[i].mod = Number(mods[i].mod) + t.mod 
      } else {
        mods.push(t)
      } 
  })
  return mods.map(talent => 
      { 
        return {
